...
export class DbContext {
    ...
    static async create(collectionName: string, documentJson: object): Promise<string> {
        try {
    ...
    static async update(collectionName: string, id: string, documentJson: object): Promise<any> {
        try {
            const collection = await DbContext.getCollection(collectionName);
            return await collection.updateOne({ _id: new ObjectId(id) }, { $set: documentJson });
        } catch (error) {
    ...
    static async read(collectionName: string, id: string): Promise<object | null> {
        try {
            const collection = await DbContext.getCollection(collectionName);
    ...
    static async readByProperty(collectionName: string, propertyValue: any): Promise<object | null> {
        try {
            const collection = await DbContext.getCollection(collectionName);
    ...
    static async readByCriteria(collectionName: string, criteria: string): Promise<object[] | null> {
        try {
            const collection = await DbContext.getCollection(collectionName);
