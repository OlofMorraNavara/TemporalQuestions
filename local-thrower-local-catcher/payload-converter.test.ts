...
describe('Test payload converter class', () => {
    ...
    test('Input payload with random class forgets class', () => {
        ...
        const converter = new TibcoPayloadConverter();
        expect({ a: {} }).toEqual(converter.fromPayload(converter.toPayload(input)));
    });
    ...
    test('Input payload with TibcoDuration class nested does not change', () => {
        ...
        const input = {
            a: { c: tibcoDuration1 },
            b: { d: { e: tibcoDuration2, f: [tibcoDuration3, tibcoDuration4] } },
        };
        const converter = new TibcoPayloadConverter();

        let output: { a: { c: any }; b: { d: { e: any; f: [any, any] } } } = converter.fromPayload(
