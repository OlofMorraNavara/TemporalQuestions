...
import { MainFlowTaskUserPageFlow } from './workflows';
...
async function run(input: WorkflowInput): Promise<WorkflowOutput> {
    ...
    const workflowId = 'task-user-page-flow-solution-' + uuid4();
    input._generated.mainParentProcessWorkflowId = workflowId;

    const handle = await client.workflow.start(MainFlowTaskUserPageFlow, {
